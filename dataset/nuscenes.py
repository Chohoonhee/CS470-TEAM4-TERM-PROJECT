...
def nuscenes_collate(batch, test_set=False):
    ...
    #print(past_agents_traj[1].shape[0])
    map_images = []
    for i in range(len(past_agents_traj)):
        for j in range(past_agents_traj[i].shape[0]):
            map_images.append(map_image[i])
    #print(map_images[0].shape)
    #print(len(map_images))
    map_images = torch.stack(map_images, dim=0)
...
class NuscenesDataset(Dataset):    
    def __init__(self, data_partition, map_version, sampling_rate, sample_stride=1,
                 use_scene=True, scene_size=(64, 64), ploss_type=None, intrinsic_rate=2,
    ...
    def __getitem__(self, idx):
        ...
        if self.use_scene:
            ...
            #cv2.imwrite("./test1.jpg", map_image)
        ...
        if 'test' in self.data_partition:
          ...
          episode = (past_agents_traj, past_agents_traj_len, future_agents_traj, future_agents_traj_len,\
                     future_agent_masks, decode_start_vel, decode_start_pos, map_image, prior, scene_id)
        return episode
    ...
    def extract_submodule_multicore(path_list, options_list):
        ...
        return (past_traj, past_traj_len, future_traj, future_traj_len, future_agent_ids_mask, decode_start_pos, decode_start_vel, scene_id), conditio
